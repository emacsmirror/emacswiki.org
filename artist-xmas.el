;;; artist-xmas.el --- xemacs port of artist.el
...
(defcustom artist-aspect-ratio 
  (/ (float (face-height 'default)) (float (face-width 'default)))
...
;;(defvar artist-pointer-shape 
;; (if (eq window-system 'x) x-pointer-crosshair nil)
...
(defun mouse-movement-p (event)
  (eq (event-type event) 'motion))


(defvar artist-mode-map
  (let ((map (make-sparse-keymap)))
    (setq artist-mode-map (make-sparse-keymap))
    (define-key map 'button1 'artist-down-mouse-1)
    (define-key map [(shift button1)] 'artist-down-mouse-1)
    (define-key map 'button2 'artist-mouse-choose-operation)
    (define-key map [(shift button2)] 'artist-mouse-choose-operation)
    (define-key map 'button3 'artist-down-mouse-3)
    (define-key map [(shift button3)] 'artist-down-mouse-3)
    (define-key map [(control button4)] 'artist-select-prev-op-in-list)
    (define-key map [(control button5)] 'artist-select-next-op-in-list)
...
(add-minor-mode 'artist-mode 'nil
                (if (boundp 'artist-mode-map)
                    (symbol-value 'artist-mode-map)))
     ...
     (let ((element-tag (artist-mt-get-tag element)))
       (cond ((eq element-tag 'graphics-operation)
	      (let* ((info-part (artist-mt-get-info-part element))
		     ...
		     (unshifted (artist-go-get-unshifted info-part))
		     (symbol    (artist-go-get-symbol unshifted)))
		(vector descr (list 'identity (list 'quote symbol)) t)))

	     ((eq element-tag 'function-call)
	      (let* ((info-part (artist-mt-get-info-part element))
		     (keyword   (artist-fc-get-keyword info-part))
		     (symbol    (artist-fc-get-symbol info-part)))
		(vector keyword (list 'identity (list 'quote symbol)) t)))
...
(defun artist-get-char-at-xy-conv (x y)
  ...
  (save-excursion
    (let ((tmp-char (aref artist-replacement-table (artist-get-char-at-xy x y))))
      (if (integerp tmp-char)
	  (int-to-char tmp-char)
	tmp-char))))
...
(defun artist-down-mouse-1 (ev)
  ...
  (let* ((real (artist-go-get-symbol-shift
	 ...
	 ;;;            (unshifted basic)
	 ;;;            (shifted 
         ;;;                 (make-symbol (concat "S-" (symbol-name basic)))))
...
(defun artist-mouse-choose-operation (ev op)
  ...
  (interactive
   (progn
     (select-window (event-window last-input-event))
     (let ((responce (get-popup-menu-response artist-popup-menu-table)))
       (list last-input-event
	     (funcall 
	      (event-function responce)
	      (event-object responce))))))

   
  (let ((draw-fn (artist-go-get-draw-fn-from-symbol op))
	(set-fn (artist-fc-get-fn-from-symbol op)))
    (cond

     ;; *** It was a draw-function
     ((not (listp draw-fn))
      (let* ((unshifted    (artist-go-get-symbol-shift op nil))
	     (shifted      (artist-go-get-symbol-shift op t))
	     (shift-state  (artist-event-is-shifted ev))
	     (selected-op  (if shift-state shifted unshifted))
	     (keyword      (artist-go-get-keyword-from-symbol selected-op)))
	(artist-select-operation keyword)))
...
The event, EV, is the mouse event."
  (let* ((unshifted    (artist-go-get-symbol-shift artist-curr-go nil))
	 ...
	 (initial-win  (event-window  ev))
	 (ev-start-pos (artist-coord-win-to-buf 
			(cons (/ (- (event-window-x-pixel ev)
				    (window-left-margin-pixel-width
				     (event-window  ev)))
				 (face-width 'default)) 
			      (/ (event-window-y-pixel ev) 
				 (face-height 'default)))))
	 (x1           (car ev-start-pos))
	 (y1           (cdr ev-start-pos))
	 (shape)
	 (timer))
    (select-window (event-window  ev))
    (artist-funcall init-fn x1 y1)
    (if (not artist-rubber-banding)
	(artist-no-rb-set-point1 x1 y1))
    (progn
      (while (or (mouse-movement-p ev)
		 (eq (event-type ev) 'button-press)
		 (eq (event-type ev) 'timeout)
		 (eq (event-type ev) 'magic))
      
	(if (not (or (eq (event-type ev) 'timeout)
		     (eq (event-type ev) 'magic)))
	    (progn

	      (setq ev-start-pos 
		    (artist-coord-win-to-buf
		     (cons 
		      (/ (- (event-window-x-pixel ev)
			    (window-left-margin-pixel-width
			     (event-window  ev))) (face-width 'default)) 
		      (/ (event-window-y-pixel ev) (face-height 'default)))))
	      ...
	      (if (not (eq initial-win (event-window ev)))
		...
		(if (artist-shift-has-changed shift-state ev)
			...
			(message "Cannot switch to shifted operation")
		      ...
		      (setq draw-how    
			    (artist-go-get-draw-how-from-symbol op))
		      (setq draw-fn     
			    (artist-go-get-draw-fn-from-symbol op))))
		...
		(if (and interval draw-fn)
		    (setq timer 
			  (run-at-time interval interval draw-fn x1 y1))))))
...
The event, EV, is the mouse event."
  ...
  (let* ((unshifted    (artist-go-get-symbol-shift artist-curr-go nil))
	...
	(cond
	 ...
	 ((and is-down
	       (or (eq (event-type ev) 'button-release)
		   ;;(eq (event-type ev) 'motion)
		   (eq (event-type ev) 'timeout)
		   (eq (event-type ev) 'magic)))
	  ...
	  (if (not (or (eq (event-type ev) 'timeout)
		...
		(if (not artist-rubber-banding)
		    (progn
		      (artist-no-rb-unset-points)
		      (setq shape 
			    (artist-funcall draw-fn x1-last y1-last x2 y2))))
	 ...
	 ((and (eq (event-type ev) 'button-press)
	       (or (equal (event-button ev) 2)
		   (equal (event-button ev) 3)))
	  nil)
	 ...
	 ((and (eq (event-type ev) 'button-release)
	       ;;(eq (event-type ev) 'motion
	       (or (equal (event-button ev) 2)
		   (equal (event-button ev) 3)))
	  (setq done t))
	...
	;;;(setq done t))
	 ...
	 ((and (not is-down)
	       (or (and (eq (event-type ev) 'button-press)
			(equal (event-button ev) 1))
		   (eq (event-type ev) 'timeout)
		   (eq (event-type ev) 'magic)))
	  ...
	  (if (not (or (eq (event-type ev) 'timeout)
		...
		(if (not (eq draw-how
		  ...
		  (progn
		    ...
		    ;;Draw new shape(if not rubber-banding,place both marks)
		    ;;
	 ...
	 ((and (mouse-movement-p ev)
	       is-down
	       (eq initial-win (event-window ev)))
	  ;; Draw shape from last place to this place
	  ;;
	  ;; set x2 and y2
	  (setq ev-start-pos 
	 ...
	 ((and (mouse-movement-p ev)
	       is-down
	       (not (eq initial-win (event-window ev))))
	  ;; Ignore
	  nil)
	 ...
	 ;; *** Another event
	 ((not (or (eq (event-type ev) 'timeout)
		   (eq (event-type ev) 'magic)))
	  ;; End drawing
	  ;;
	  (setq done t)))
		...
		(if (artist-shift-has-changed shift-state ev)
			...
			(message "Cannot switch to shifted operation")
		      ...
		      (setq draw-fn     
			    (artist-go-get-draw-fn-from-symbol op))
		      (setq undraw-fn   
			    (artist-go-get-undraw-fn-from-symbol op))
		      (setq fill-pred   
			    (artist-go-get-fill-pred-from-symbol op))
		      (setq fill-fn     
			    (artist-go-get-fill-fn-from-symbol op))))

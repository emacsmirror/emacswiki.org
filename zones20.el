...
;;
;;    If you have Emacs 22 or later, use library `zones.el' instead.
;;    Library `zones20.el' provides only some of its features, for use
;;    with Emacs 20 or 21.
;;
...
;;    `zz-narrow', `zz-select-region', `zz-set-izones-var',
...
;;    `zz-markerize', `zz-max', `zz-min', `zz-number-or-marker-p',
...
;;    `zz-remove-zones-w-other-buffer-markers', `zz-set-intersection',
;;    `zz-set-union', `zz-some', `zz-string-match-p',
;;    `zz-two-zone-intersection', `zz-two-zone-union',
;;    `zz-zone-buffer-name', `zz-zone-has-other-buffer-marker-p',
;;    `zz-zone-intersection', `zz-zone-intersection-1',
;;    `zz-zone-ordered', `zz-zones-complement', `zz-zones-overlap-p',
...
;;  -------------
;;
;;  Library `zones20.el' lets you easily define and subsequently act on
...
;;  C-x n r   `zz-select-region' - Select a region from set of zones
;;  C-x n u   `zz-unite-zones' - Unite (coalesce) izones
;;  C-x n v   `zz-set-izones-var' - Set `zz-izones-var' to a variable
;;  C-x n w   `widen'
;;  C-x n x   `zz-narrow' - Widen to a previous buffer restriction
;;
;;  (You need library `hlt-highlight.el' to be able to use `C-x n h',
;;  `C-x n H', `C-x n l', and `C-x n L'.)
;;
;;
;;  ** Command `zz-narrow' **
;;
;;  Library `zones20.el' modifies commands `narrow-to-region',
...
;;
;;  You can then use `C-x n x' to change to a previous buffer
;;  restriction (narrowing).  This gives you an easy way to browse
;;  your past narrowings.
;;
;;  If the izone variable is not buffer-local then `zz-narrow' lets
;;  you choose among the narrowings in different buffers, switching
...
;;
;;  By default, `C-x n x' is bound to command `zz-narrow'.
;;
...
;;  takes the region as an argument.  What `zones20.el' offers in this
...
;;
;; 2018/10/29 dadams
;;     Forked from zones.el, for Emacs 20-21 only.
;;      Removed: zz-fringe-for-narrowing, zz-narrowing-use-fringe-flag, zz-set-fringe-for-narrowing,
;;               zz-narrowing-lighter, zz-user-error, zz-regexp-car-member, zz-map-query-replace-regexp-zones,
;;               zz-replace-(string|regexp)-zones, zz-query-replace(-regexp)-zones,
;;               zz-(i)zones-from-noncontiguous-region, zz-noncontiguous-region-from-(i)zones, zz-dot-pairs.
;; 2018/10/28 dadams
...
;;;###autoload
(defun zz-select-region (arg &optional msgp) ; Bound to `C-x n r'.
...
;;
;;;###autoload
(defun zz-narrow (arg &optional msgp) ; Bound to `C-x n x'.
 ...
 the ring entries from the most recent back through the (-)Nth one."
  ...
  (let* ((var  zz-izones-var)
    ...
    (cond ((or (consp arg)  (and (null (cdr val))
                                 (/= (- (point-max) (point-min)) (buffer-size)))) ; Buffer is narrowed.
           (widen)
  ...
  (let ((bufs  ())
    ...
    (while (condition-case nil
               (setq buf  (read-buffer "Buffer (C-g to end): "
                                       (and (not (member (buffer-name (current-buffer)) bufs))  (current-buffer))
                                       t))
             (quit nil))
      (push buf bufs))
  ...
  default (nil), check property `face'."
    (interactive
     (let ((numarg  (and current-prefix-arg  (prefix-numeric-value current-prefix-arg))))
       (unless (require 'highlight nil t) (error "You need library `highlight.el' to use this command"))

...
(defface zjl-hl-function-call-face
  '((((class color)
    ...
    (((class color)
      (background light))
     (:foreground "#008000" :bold t);theme 0
...
(defface zjl-hl-parameters-reference-face
  '((((class color)
    ...
    (((class color)
      (background light))
     (:foreground "#008080" :bold t));theme 0
     ;; (:foreground "#008000" :bold t)) ;;theme1
    (t
     ()))
    ...
    (let* ((local-variable-list '()) (local-constant-list '())
      ...
      (condition-case nil         
          (when func-body-begin                               
            ...
            (when items
              (dolist (item items)
                (when (not (memq (car item) parameter-list))
                  (setq parameter-list (cons (car item) parameter-list)))
                ))
            
            (save-excursion
              (when parameter-list ;; should before local-variable-list since some parameter will override by local-variable
                (case major-mode
      ...
      (condition-case nil
          (when func-body-begin                               
            ...
            (while (progn (setq temp-value (zjl-hl-find-regexp-return-end-pos2 (point) end "\\(\\_<return\\_>\\)\\|\\(\\(?:volatile +\\)?\\_<\\(?:\\w\\|\\s_\\)+\\_>\\(?:[ ]+\\**\\|\\**[ ]+\\)\\(?:\\_<\\(?:\\w\\|\\s_\\)+\\_>\\)[ 
              ...
              (when (cdr-safe temp-value);; thus skip the "..." and the return.. skp the ".." is because the (blackward-up-list) will crash if the point is in the "..".    NUMBER 12 here is because the zjl-xxxx-pos2 is has a \\(in it)
                ...
                (when (not (memq subdomain-start domainstartlist))
                  ...
                  (when items
                    (dolist (item items)
                      (when (and (not (memq (car item) local-variable-list))
                                 (not (memq (car item) local-constant-list));;if use the parse-region then need this line
                                 )
        ...
        (error (setq successful nil)))
      successful))
  (error (message "zjl-hl-local-variable-and-parameter-in-func-region error")))
  )
                ...
                (when (and (< (- end start) zjl-hl-toobig-size)
                  ...
                  (when temp-regions
                    (dolist (each-region temp-regions)
                      ...
                      (setq zjl-hl-regions-already-hl (region-list-edit-add zjl-hl-regions-already-hl each-region t)) ;; t to merge hl-regions;; even we met failure in highlight region, still add it into DONE region, no repeat (slow)
                      )

I found that this program did not work for me. I modified it a little bit to actually correctly break when it found my signature. However I broke the feature that it seems to want to have of saving your /last/ signature.
  ...
  (let* ((elist mail-signature-alist)
    ...
    (save-excursion
      ...
      (while (and (not found) elist)
        (cond ((and (mail-position-on-field header 'soft)
                    (re-search-backward (concat header ":[ \t]*\\(.*\\)$")
                                        (point-min) t)
                    (string-match regex (buffer-substring-no-properties (match-beginning 1)
                                                                        (match-end 1))))
               ;; found it
               (setq found t))
              ((<= sind mail-signature-last-signature) (setq elist (cdr elist)
                                                             entry (car elist)
                                                             header (car entry)
                                                             sind (1+ sind)
                                                             regex (car (cdr entry))
                                                             file (car (cdr (cdr entry)))))
              ;; we break the loop and use the default
              (t (setq found t)))))
...
-- [https://what-i-am-upto.neocities.org/ jordanr] 2023-10-01 21:25 UTC
...
-- [https://what-i-am-upto.neocities.org/ jordanr] 2023-10-01 21:26 UTC

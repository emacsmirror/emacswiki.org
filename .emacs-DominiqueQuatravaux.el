;;; -*- emacs-lisp -*-
;; .emacs, fichier de configuration d'emacs de Dom - $Revision: 1.262 $
;;
;; MODE D'EMPLOI
;;
  ...
  (let* ((mes-chemins
          (funcall #'nconc
                   ...
                   (mapcar (lambda (subdir)
                           ...
                           (list emacs-version "magit" "nxhtml/alts"
                                 "el-get/el-get"))
...
(defvar pointemacs-chemin-autostart-nxhtml
  (expand-file-name (concat pointemacs-repertoire-lib
                            "/nxhtml/autostart.el"))
...
(pointemacs-personnalise '(rng-schema-locating-files
        (list
...
;; (http://stackoverflow.com/questions/1431843)
(global-set-key [(f5)] 'font-lock-fontify-buffer)
(eval-after-load "mmm-mode"
   '(define-key mmm-mode-map [(f5)] 'mmm-parse-block))

(eval-after-load "nxhtml-mode"
  '(define-key nxhtml-mode-map (kbd "C-c /") 'rngalt-finish-element))
...
(pointemacs-personnalise '(revert-without-query
...
(when (and (string-lessp "22" emacs-version)
  ...
  (pointemacs-personnalise '(nxhtml-skip-welcome t)
  ...
  (require-faible 'ourcomments-widgets)
...
(pointemacs-personnalise '(safe-local-variable-values (quote ((auto-fill-mode)))))
...
;; Grep en couleurs (mais voir plus bas au sujet de pcregrep) :
...
(eval-after-load "grep" '(progn
     ...
     ;; Utilisation de pcregrep avec couleurs si possible
     (when (pointemacs-choisir-commande "pcregrep")
       (let ((grep-program "pcregrep"))
         (grep-compute-defaults)
         (let ((reglages-localhost
                (cdr (assq 'localhost grep-host-defaults-alist))))
           (dolist (reglage '(grep-command grep-template grep-find-template))
             (let ((place (cdr (assq reglage reglages-localhost))))
               (setcar place (replace-regexp-in-string
                              "pcregrep" "pcregrep --color=auto" (car place)))))
           (grep-compute-defaults))))))
...
;; Tramp : accès à des fichiers à distance. Marche avec le contrôle de version
;; et M-x grep !!

;; Accès par ssh + sudo selon /sudo:root@kilim:/etc/passwd
;; http://www.emacswiki.org/emacs/TrampMode#toc12
(pointemacs-personnalise '(tramp-default-proxies-alist
  '(("kilim" "root" "/ssh:dom@kilim:")
    ("cmisrvm2" "root" "/ssh:quatrava@cmisrvm2:")
    ("stiinstallsrv" "root" "/ssh:dom@stiinstallsrv:")
    ("stisrv6" "root" "/ssh:dominique@stisrv6:"))))

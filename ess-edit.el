...
; -------------------wrappers for emacs-xemacs comp-------------------
...
(defun ess-edit-insert-debug-lines ()
  (interactive)
  (goto-char (point-min))
  (let ((i 1))
    (while (not (eobp))
      (insert "message(paste(\"line\"," (int-to-string i) "))\n")
      (forward-line 1)
      (setq i (+ 1 i)))))


(defun ess-edit-remove-debug-lines ()
  (interactive)
  (goto-char (point-min))
  (while (re-search-forward "^message(paste(\"line\",[0-9]+))$" nil t)
    (replace-match "")
    (kill-entire-line)))

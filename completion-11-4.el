...
;;; Byte-compile it using emacs -q (i.e. without having this file loaded),
;;; and load cc-mode first if you use C, or you will get errors in cc-mode
;;; like this:
;;;   Wrong type argument: commandp, c-electric-semi&comma
;;;
;;;
;;;				NO WARRANTY
;;;
;;; This software is distributed free of charge and is in the public domain.
;;; Anyone may use, duplicate or modify this program.  
;;; 
;;; The author(s) provide absolutely no warranty of any kind. The entire 
;;; risk as to the quality and performance of this program is with 
;;; you.  In no event will the author(s) be liable to you for
...

;;; Authorship:
;;;     Jim Salem      {salem@newview.org}
;;;  Inspired by work by:
;;;     Brewster Kahle {brewster@archive.org}
;;;
;;; Availability
;;;--------------
;;; http://www.newview.org/salem/completion
;;;

;;;---------------------------------------------------------------------------
...
;;;-----------------------------------------------
;;;
;;;  Should run on 18.49, 18.52, 18.55, 18.56, 19.19, 19.30 and 19.34, 20, 21.2.1, and 22.0.
...
;;;   Put Doc in texinfo format
;;;   Make it easier to change variable definitions while still using
;;;      compile time expansion to improve performance
;;;   Completion advise should be entirely installed at run time rather
;;;     than being compiled at compile time (see mail message)
;;;
;;;-----------------------------------------------
;;; History ::
;;;-----------------------------------------------
...
;;;-----------------------------------------------
;;;
;;;  The other known bugs are on some Motorola RISC platforms running Sys V
...
;;; Change Log
;;;-----------------------------------------------
;;; From version 11.3 to 11.4 (garyo@oberbrunner.com):
;;;  - handle scan-sexp errs in cmpl-scan-sexps 
;;;  - use string-to-number instead of obsolete string-to-int
;;;  - use buffer-substring-no-properties to avoid copying/saving string props
;;;  - use cc-mode hooks to set up C and C++ modes better
;;;  - use current-time to get hours-since-1900 (faster & simpler)
;;;  - document how to byte-compile properly
;;;  - add optional yank-handler to patched kill-region
;;;
;;; From version 11.2 to 11.3
;;;  - Fixes for emacs version 21 and beyond
;;;  - Fix C mode bug.  Use run-at-time instead of the older call to wakeup, etc.
;;;
;;; From version 11.1 to 11.2
;;;  - Minor bug fix for 19.34
;;;
;;; From version 11.0 to 11.1
;;;  - Update for 19.30
;;;  - Remove CM Fortran and C* modes
;;;
;;; From version 10.5 to 11.0
;;;  - Prepare for new public release
;;;  - Fix problem with def-completion-wrapper and autoload functions
;;;  - Removed "<" and ">" from the list of standard symbol characters
;;;
;;; From version 10.4 to 10.5
;;;  - Ported to 19.19
;;;  - Removed kludges that were necessary for Emacs 19.0 
;;;       (they didn't work in 19.19)
;;;  - C++ support added
;;;  - Fixed autofilling for Emacs 19.0
;;;
;;; From version 10.3 to 10.4
;;;  - Minor fix to *completion-idle-program* initialization for Epoch 4.0
;;;  - Fixed *completion-prefix-min-length* mechanism (previously the value
;;;    3 was hardcoded in completion-search-reset)
;;;  - Added (provide 'completion)
;;;  - Added a separate entrypoint for cdabbrev
;;;  - Renamed meta-/ to cdabbrev since people complained about rebinding
;;;    standard keys.  cdabbrev seemed to be a good compromise.
;;;  - Fixed autofilling to work with Lucid Emacs' name change of auto-fill-hook
;;;    to auto-fill-function and (hopefully) its byte-compiler
;;;
...
(defmacro read-time-eval (form)
  ;; Like the #. reader macro in Common Lisp
  (eval form)
  )
...
(defmacro cmpl-apply-as-top-level (function arglist)
  ...
  (list 'cmpl-apply-as-top-level-1 function arglist
	;; One would think the following line would work, however, the 
	;; clever (;-{) byte compiler in 19.22 optimizes away the binding 
	;; of executing-kbd-macro.  Aargh!!!
	;;'(let ((executing-kbd-macro nil)) (interactive-p)))
...
(defun completion-advise-1 (function-name where body &optional new-name)
  ;;; (message (concat "Adding advice for " function-name)
...
(defun completion-advise-do-autoload
  ...
  ;; This assumes that 
  ;;    (1) replace-name is a var. containing the list (autoload filename...)
  ;;    (2) loading filename defines function-name
  ;;
  ;; It loads the file, redefines the advice and calls it.
  (interactive) ;; Necessary to to cmpl-apply-as-top-level correctly
  (let ((filename (second (symbol-value replace-name)))
    ...
    ;; Load the file
    (load filename)
    ;; Verify that it's not defined
    (cond ((not (function-defined-and-loaded function-name))
	   ;; Hmmm... autoload seems to have failed.  
	   ;; Make sure we make it clear to the user.
	   (error "The autoload file %s did not define the function %s"
		  filename function-name)
	   )
	  (t
	    ;; Everything is going well, reinstall advise
...
;;; Tests -
;;;(hours-since-1970 '(0 0 0))                  --> 0.0
;;;(hours-since-1970 '(17680 1614 928000))      --> 321855.026
;;;(cmpl-hours-since-1900 '(0 0 0))             --> 613606
;;;(cmpl-hours-since-1900 '(17680 1614 928000)) --> 935461
...
(defun make-standard-completion-syntax-table ()
  (let ((table (copy-syntax-table))
    ...
    ;; Clear everything to whitespace
    (dotimes (i 256)
...
(defun symbol-before-point-for-cdabbrev ()
  ;; "Returns a string of the symbol immediately before point
  ;; or nil if there isn't one.  Like symbol-before-point-for-complete but
  ;; doesn't have a minimum length."
  ;; Cursor is on following-char and after preceding-char
  (setq cmpl-saved-syntax (syntax-table))
  (set-syntax-table cmpl-syntax-table)
  (cond ((memq (setq cmpl-preceding-syntax (char-syntax (preceding-char)))
...
;; Define this here to avoid warnings
...
"
  ...
  (let* ((print-status-p
    ...
    ;; If found, insert
    (cond (entry
	   ;; Setup for proper case and insert
...
(defun locate-completion-entry-retry (old-entry)
  ;; This is called when the database is messed up.  It re-adds the
  ;; completion to the database and then tries to locate it.
...
;;; Redefine meta-/ since cdabbrev does it one better.
...
(defun completion-separator-self-insert-autofilling (arg)
  ...
  (and (> (current-column) fill-column)
	   ...
	   ;; This version is for Lucid's emacs and Emacs 19
...
(if (>= cmpl-emacs-major-version 19)
    ...
    ;; On some older versions this is needed (because self-insert only auto 
    ;; fills if last-command is self-insert-command) 
...
;; This seems to break with newer versions of 
